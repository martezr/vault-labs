    ...
    stages {
        ...
        stage('Provision Vault Instance'){
            steps {
                ...
                sh 'sudo curl --header "X-Vault-Token:vaultpassword" --request PUT --data @policies/vault-admin-policy.json http://127.0.0.1:8200/v1/sys/policy/vault-admin-policy'
            }
        }
